...
#
# Requirements:
# - rofi (https://github.com/stevenliebregt/rofi-scripts)

# Theme of rofi got from adix github
dir="$HOME/.config/rofi/launchers/text/style_3.rasi"
...
SITES=(
    ...
    ["arch pakages"]="https://aur.archlinux.org/packages?K="
...
#     ["  duckduckgo"]="https://duckduckgo.com/?q="
#     ["  superuser"]="https://superuser.com/search?q="
#     ["爵  startpage"]="https://www.startpage.com/sp/search?query="
#     ["  arch pakages"]="https://aur.archlinux.org/packages?K="
#     ["  github "]="https://www.github.com/search?q="
#     ["  youtube"]="https://www.youtube.com/results?search_query="
#     ["磌  wikipedia"]="https://en.wikipedia.org/wiki/"
# )
...
handle_query()
{
    ...
    $BROWSER "${SITES[$site]}$query&hl=en"
}
...
main()
{
    ...
    then 
        ...
        if [ "$SITE_TO_USE" ]; then
            query=$(echo "$(gen_queries_list  $SITE_TO_USE)" | rofi -dmenu -p "    ($SITE_TO_USE) " -theme $dir)

            handle_query "$SITE_TO_USE" "$query"
        else
            site=$(echo "$(gen_sites_list)" | rofi -dmenu -p "  " -no-custom -theme $dir)
            query=$(echo "$(gen_queries_list $site)" | rofi -dmenu -p " ($site)" -theme $dir)
